...
export default {
  ...
  methods: {
    ...
    async fetchStats() {
      try {
        const response = await http.get('/api/v1/mini_core/distribution/income/stats', this.buildSearchParams())
    ...
    async fetchData() {
      this.loading = true
      try {
        const response = await http.get('/api/v1/mini_core/distribution/income', this.buildSearchParams())
